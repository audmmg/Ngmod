...
class tvscat  extends tvsclass{
    ...
    */
    ...
    private $CATKrovinioUzsakLinkLive = "https://viawebr.groupecat.com/webservices/WsComEdiWebService"; 
    private $CATKrovinioUzsakLinkDemo = "https://viawebr7.groupecat.com/webservices/WsComEdiWebService"; 
    
    private $CATKrovinioUzsakRezimLive = "0";// LIVE rezimas
    private $CATKrovinioUzsakRezimDemo = "1";// TEST rezimas
...
*/
    ...
    public $CATXML = '';
    public $CATXML_created = false;
    public $CATXML_result = "";
    ...
    function __construct() {//mode = 'test'; //test/live
        ...
        if($this->mode=='live'){
            ...
            $this->CATKrovinioUzsakLink = $this->CATKrovinioUzsakLinkLive;
            $this->CATKrovinioUzsakRezim = $this->CATKrovinioUzsakRezimLive;

        }elseif($this->mode=='test'){

            $this->CATKrovinioUzsakLink = $this->CATKrovinioUzsakLinkDemo;
            $this->CATKrovinioUzsakRezim = $this->CATKrovinioUzsakRezimDemo;

        }else{
            $this->mode = 'notSet';

            $this->CATKrovinioUzsakLink = '';
        }
    ...
    function setSiuntaData($SiuntaUID, $sParam) {//siuntos UID DB ir sParam - duomenys is WEB formos
        ...
        $this->Sandelys = 'NaN';
        if($this->sParam['sCAT_SandelysKEGKPG']=='KEG'){
            $this->sandelys = 'KEG';
        }else if($this->sParam['sCAT_SandelysKEGKPG']=='KPG'){
            $this->sandelys = 'KPG';
        }else if($this->sParam['sCAT_SandelysKEGKPG']=='ETK' OR $this->sParam['sCAT_SandelysKEGKPG']=='ETK1'){
            $this->sandelys = 'ETK';
        }else{
            $Error['message'] = 'Nenustatyta iš kurio sandėlio bus siunta';
            $Error['code'] = "TAD-C2011"; //t-transport AD-address data
        ...
        if($this->sandelys == 'ETK'){
            //Disablinam, nes CAT tik is KPG sandelio
            $SiuntaDataRez = array();
        ...
        if($SiuntaDataRez['OK']=='OK' AND ($this->mode == 'live' OR $this->mode == 'test')){
            ...
            if($this->SiuntaData['NeutralumasKod'] == ''){
                    ...
                    */
                    $this->tvs['neutralShipping']=0;//cia visi be neutralumo(auksciau if'as)
    ...
    public function setPackBasic ($SiuntaData){
        ...
        if ($SiuntaData){
            ...
            if (!$SiuntaData['SvorisSum']){
                ...
                $Error['code'] = "TPD-2107"; //t-transport AD-address data
            ...
            if($pakSkaicius==0){
                ...
                $Error['code'] = "TPD-2108"; //t-transport AD-address data
                $Error['group'] = "PD"; //PD - pack data error
                $this->addError ($Error);
            }
            ...
            */
            ...
            if($this->haveErrors ('PD')==0){
                ...
                $Error['code'] = "TAD-2110"; //t-transport AD-address data
                $Error['group'] = "PD"; 
                $this->addError ($Error);
    ...
    public function generateCATXML ($param=array()){

        //param bus tuscias nes visus duomenys ims is saves (is saco classes anksciau nustatytu duomenu)
        ...
        $GavejoPapildomaInfo = ' '.$this->addres['CONSIGNEE']['contactPerson']['name'].', Phone.: '.$phone.', e-mail: '.$email;

        $tomorrow = date('Y-m-d', strtotime('+1 day'));
        $JSON_cat = '<?xml version="1.0" encoding="utf-8"?>
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://com.webservices.customservices.catvs.wexlog.com/">
   <soapenv:Header/>
   <soapenv:Body>
      <com:registerIncomingIfcsumManifest>
         <arg0>
            <loginData>
               <organisationUnitId>LTVILF</organisationUnitId>
               <userId>AURIKAWS</userId>
               <userPassword>AUrikaA2022!</userPassword>
            </loginData>
            <CatIfcsumIn>
                <cusords>
                    <sndId>AURILT</sndId>
                    <sndName>AURIKA UAB</sndName>
                    <sndAddress>'.$this->addres['SHIPPER']['street'].'</sndAddress>
                    <sndAddress2></sndAddress2>
                    <sndZip>'.$this->addres['SHIPPER']['postalCode'].'</sndZip>
                    <sndCity>'.$this->addres['SHIPPER']['city'].'</sndCity>
                    <sndCountry>'.$this->addres['SHIPPER']['countryCode'].'</sndCountry> 

                    <cgnId></cgnId>
                    <cgnName>'.$this->addres['CONSIGNEE']['name1'].'</cgnName>
                    <cgnAddress>'.$this->addres['CONSIGNEE']['street'].'</cgnAddress>
                    <cgnAddress2>'.$this->addres['CONSIGNEE']['street2'].'</cgnAddress2>
                    <cgnCity>'.$this->addres['CONSIGNEE']['city'].'</cgnCity>
                    <cgnCountry>'.$this->addres['CONSIGNEE']['countryCode'].'</cgnCountry>
                    <cgnZip>'.$this->addres['CONSIGNEE']['postalCode'].'</cgnZip>
                    <estTimeDep>'.$tomorrow.'T00:00:00</estTimeDep>
                    <firstOperationType>PKP</firstOperationType>
                    <lastOperationType>DEL</lastOperationType>
                    <gdsDescription>'.$GavejoPapildomaInfo.'</gdsDescription>
';        


if($this->packs['PacksArray']){

    $siandien = date("ymd"); // 241120
    ...
    foreach ($this->packs['PacksArray'] as $Pakkey => $pakis) {


                    $randomNumber = random_int(10000, 99999); // atsitiktinis skaicius is 5 skaitmenu

                    //$parcel = $param['parcel'];
                    //$addres = $param['addres'];

                    //$parcel['dimTuris'] = $PakValue['Turis'];// cia turis m3 //($PakValue['Plotis'] * $parcel['Ilgis'] * $parcel['Aukstis']) / 1000000;
                    $dimTrackingNr = 'AURILT'.$siandien.$randomNumber; // vietoj random reikutu imti is DB parametrines lenteles is eiles skaicius
                    ...
                    //foreach ($this->packs['PacksArray'] as $key => $pakis) {
                        ...
                        if ($pakNo > 1){
                            //$JSON_cat .= ', '; //cia nereikia kablelio
                        }
...
';
                                    ...
                                    //tracking number
$JSON_cat .= '                      <barcodeExtId>'.$dimTrackingNr.'</barcodeExtId>
';


                                    //siuntos parametrai
$JSON_cat .= '                      <dimHeight>'.$pakis['Aukstis'].'</dimHeight>
                      <dimLength>'.$pakis['Ilgis'].'</dimLength>
                      <dimWidth>'.$pakis['Plotis'].'</dimWidth>
                      <grsWgt>'.$pakis['Svoris'].'</grsWgt>
';


                                    if($pakis['Tipas']=='EP' OR $pakis['Tipas']=='MP' ){// jeigu paletes tai rasom prie UNIT
$JSON_cat .= '                      <huType>LTCO</huType>
';
...
$JSON_cat .='                    
                    <sprId>EUROCAT_ATA</sprId>
                    <xaccountId>AURIKA_LT</xaccountId>
                </cusords>
            </CatIfcsumIn>
         </arg0>
      </com:registerIncomingIfcsumManifest>
...
';
        ...
        $this->CATXML = $JSON_cat;

        if($pavyko===true){
            $this->CATXML_created = true;
        }else{
            $this->CATXML = '';//nunulinam jeigu kazkur kazkas nepavyko
            $this->CATXML_created = false;
        }

        //var_dump($this->CATXML);
...
//TEST SO 312447 LS
    ...
    private function connectSend(){
        ...
        $ch = curl_init($this->CATKrovinioUzsakLink);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: text/xml','charset=UTF-8'));
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $this->CATXML);
        ...
        if($output){
            if( strpos($output, '500 Internal Server Error') !== false OR  strpos($output, 'HTTP GET not supported') !== false){
                $resperror = 3;//3- klaida response registruojant siunta
                $bookingId = '';
                $Error['message'] = "Gauta klaida 500 Internal Server Error";
                $Error['code'] = "TSX-2085"; //t-transport SX-send XML
                ...
                $this->addError ($Error);
                    ...
                    //parsinam XML
                    if(isset($RespArray['ns2registerIncomingIfcsumManifestResponse']['return'])){        
                        if ($RespArray['ns2registerIncomingIfcsumManifestResponse']['return']['isProcessed']=='true' AND $RespArray['ns2registerIncomingIfcsumManifestResponse']['return']['retCode']['ok']=='true'){//jeigu grizo geras atsakymas, 
                                    $resperror = 1;//1- nera klaidos
                                    $respar = $RespArray['ns2registerIncomingIfcsumManifestResponse']['return'];
                                    $requestID = $respar['sessionId'];
                                    $bookingId = $respar['catifcsumOut']['cusord']['ordId'];
                                    //var_dump($resp);
                            ...
                            $resperror = 3;//3- klaida response registruojant siunta
                            $bookingId = '';
                            $ErrorsArray = $RespArray['ns2registerIncomingIfcsumManifestResponse']['return']['retCode']['messages'];
                            if(is_array($ErrorsArray)){
                                foreach ($ErrorsArray as $Ekey => $Evalue) {
                                    $Error['message'] = $Evalue['text'];
                                    $Error['code'] = "TSX-2152"; //t-transport SX-send XML
                                    $Error['group'] = "SX"; //SX-send XML error
                                    $this->addError ($Error);
                                }
                            }else{
                                $Error['message'] = "Nežinoma klaida registruojant siuntą.";
                                $Error['code'] = "TSX-2151"; //t-transport SX-send XML
                                $Error['group'] = "SX"; //SX-send XML error
                                $this->addError ($Error);
                            }//end else
                        ...
                        $Error['message'] = "Klaida iš CAT registruojant siuntą: ".$errorComment;
                        $Error['code'] = "TSX-2152"; //t-transport SX-send XML
                        $Error['group'] = "SX"; //SX-send XML error
                        $this->addError ($Error);
                        ...
                        $resperror = 3;//3- klaida response registruojant siunta
                        $bookingId = '';
                        $Error['message'] = "Nežinoma klaida registruojant CAT siuntą.";
                        $Error['code'] = "TSX-2153"; //t-transport SX-send XML
                        $Error['group'] = "SX"; //SX-send XML error
                        $this->addError ($Error);
                ...
                $resperror = 3;//3- klaida response registruojant siunta
                $bookingId = '';
                $Error['message'] = "Negautas atsakymas iš CAT.";
                $Error['code'] = "TSX-2154"; //t-transport SX-send XML
                $Error['group'] = "SX"; //SX-send XML error
                $this->addError ($Error);
    ...
    public function sendXML ($param=array()){
        ...
        if ($this->CATXML_created){
            //if($this->AccessKey){
                //echo "<br>----444---";
                if($this->CATKrovinioUzsakLink){
                    //bandom registruoti
    ...
    /* pasiruosimas XML siuntimui, siuntimas ir rezultatu apdorojimas */
    public function sendXML____OLD ($SiuntaData=array()){
...
//var_dump($SiuntaData);
        ...
        $url = $this->CATKrovinioUzsakLink;

 
        $header_data[]="Content-Type: text/xml; charset=utf-8";
        ...
        $header_data[]="SOAPAction:http://com.webservices.customservices.catvs.wexlog.com/WsComEdiWebService/registerIncomingIfcsumManifest";




        echo"<br><br><br>CATXML<hr>";
        var_dump($this->CATXML);
        echo"<br><br>2222<br>URL: $url<br> URL2: ".$this->CATKrovinioUzsakLink."<hr>";


        $resp = $this->CallAPI($method, $url, $header_data, $this->CATXML);
...
*/
    ...
    public function RotateGif____OLD ($gifLocation){
    ...
    private function CreatePdf_____OLD ($siuntaID, $GifArray){
    ...
    private function CreateUPSManifestPdf____OLD ($DataArray){
    ...
    public function CreatePdfGif____OLD ($siuntaID, $HTMLtoPDF){
    ...
    public function CreatePdfManifest____OLD ($HTMLtoPDF, $date){
    ...
    public function ajaxCreateLabelPDF____OLD ($SiuntaUID){
        ...
        function CallAPI____OLD($method, $url, $header_data, $data = false)
        {
            $curl = curl_init();
        ...
        function savePdf____OLD($pdf_string, $output_file) {
        ...
        function showPdf_____OLD ($BarcodeFile){
    ...
    private function base64_to_jpeg_____OLD($base64_string, $output_file) {
    ...
    public function PrintManifest____OLD($Sandelys = 'KEG'){
