...
class pretenzijaEditController extends controller {
...
function __construct() {
    ...
    require_once ( $root_path. "modules/nz/teises.mod.php");
    $this->teises = new teises_mod();
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if($this->getVar('save')==1)$this->pretenzijaMod->editPretenzija();
        ...
        $info=$this->pretenzijaMod->getPretenzijaInfo($this->getVar('uid'));
        $this->viewer->assign('info',$info);
        $this->viewer->assign('asmenys',$this->getAsmenys());
        $this->viewer->assign('priezastys',$this->pretenzijaMod->priezastysList($this->getVar('uid')));  
        if($this->getVar('nz_tipas')==2)
        $this->viewer->assign('incidentas',"1");
        
        /***teises***/
        $this->viewer->assign('is_admin',$this->teises->is_admin(SESSION::getUserID()));
        /**teises**/
        
        $filename=dirname(__FILE__) . "/../../uploads/pretenzija".$info['dokumento_nr']."/rootcause".$info['dokumento_nr'].".pdf";
        ...
        $this->viewer->assign('visit',$this->pretenzijaMod->getVisit($info['uid'])); 
        
        
        // Load Viewer:
        if($this->getVar('nz_tipas')==1 || $this->getVar('nz_tipas')==2)
        $list_html = $this->viewer->fetch('nz/pretenzija/vidine/edit.tpl');
        else
        $list_html = $this->viewer->fetch('nz/pretenzija/edit.tpl');
...
if (@$_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest') {
    $controller = new pretenzijaEditController();
    $controller->run();
} else {
