...
class uzduotysListController extends controller {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        //$uzduotys=$this->asmenysMod->getAllUzduotys($_POST['filter']);
        
        if($_POST['export']==1){
            $f=$_POST['filter'];
            $f['per_page']=9999999;
            $f['page']=1;
            $uzduotys=$this->asmenysMod->getAllUzduotys($f);
        }
        else{
            $uzduotys=$this->asmenysMod->getAllUzduotys($_POST['filter']);  
        }
        ...
        $this->viewer->assign('uzduotys',$uzduotys['list']);
        $this->viewer->assign('filter',$uzduotys['filter']);
        $this->viewer->assign('filternames',$uzduotys['filternames']);
        // Load Viewer:
        
        
        $list_html = $this->viewer->fetch('nz/asmenys/uzduotys-list.tpl');
        $this->viewer->clearAllAssign();
        
        if($_POST['export']==1){
            //var_dump($uzduotys['list']);die;
        $this->viewer->assign('uzduotys',$uzduotys['list']);
        $html = $this->viewer->fetch('nz/asmenys/export.tpl');
...
if (@$_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest') {
    $controller = new uzduotysListController();
    $controller->run();
} else {
